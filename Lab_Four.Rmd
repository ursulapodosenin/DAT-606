...
ggplot(data=mcdonalds, aes(x=cal_fat))+
   geom_bar(color= "black", fill= "blue", size= .75)+
   theme_bw()

ggplot(data=dairy_queen, aes(x=cal_fat))+
  geom_bar(color= "black", fill= "red", size= .75)+
   theme_bw()
...
# According to the bar plot, Mcdonalds seems to have a positively skewed distributon, while Dairy Queen appears to be bimodal. This is substantiated by the box plots where Mcdonalds where the mean of the data is far to the left and there are several outliers that extend far off to the right. For Dairy Queen's box plot, the data is slightly more centered than Mcdonalds, but the mean isnt centered.
```
...
While the data appear to follow a nearly normal distribution, I would err on the side of caution and say the distribution is slightly, positively skewed due to its longer right tail.
...
ggplot(dairy_queen, aes(sample = sim_norm)) + 
  geom_line(stat = "qq")

# The sim_norm plot is closer to a diagonal line than the dairy_queen plot. This suggests that sim_norm is closer to the normal distribution than dairy queen, which appeared to be bi-modal. 
```
...
Yes, the normal probability plot for the calories from fat looks similar to the plots created for the simulated data. Some of the simulations look similar to the normal probability plot, suggesting that the data is nearly normal. 
...
mcdonaldsmean <- mean(mcdonalds$cal_fat)
mcdonaldssd <- sd(mcdonalds$cal_fat)

ggplot(data = mcdonalds, aes(sample = cal_fat)) + 
  geom_line(stat = "qq")

sim_norm <- rnorm(n = nrow(mcdonalds), mean = mcdonaldsmean, sd = mcdonaldssd)

qqnormsim(sample = cal_fat, data = mcdonalds)

# Unlike Dairy Queen, the distribution from Mcdonalds is definitely not normal. According to the Q-Q plot, the line is nowhere near diagonal, as it dips on the right side. Additionally, the Q-Q plot doesn't look anything like the simulations. Therefore, the Mcdonalds distribution is not normal. 
```
...
What is the probability that the number of calories from fat at Dairy Queen are less than 100?

```{r}
1 - pnorm(q = 100, mean = dqmean, sd = dqsd)

dairy_queen %>% 
  filter(cal_fat > 100) %>%
  summarise(percent = n() / nrow(dairy_queen))
```
What is the probability that the number of calories from fat at Dairy Queen is equal to 280?

```{r}
1 - pnorm(q = 280, mean = dqmean, sd = dqsd)

dairy_queen %>% 
  filter(cal_fat == 280) %>%
  summarise(percent = n() / nrow(dairy_queen))
```
Between the two methods, the theoretical normal distribution had a closer agreement between the two methods. 
...
ggplot(data= chickfila, aes(x=sodium))+
   geom_bar(color= "black", fill= "blue", size= .75)+
   theme_bw()
...
ggplot(data= taco_bell, aes(x=sodium))+
   ...
   theme_bw()
```
Of all the resteraunts, Burger King has the most normal sodium distribution.
...
There might be a step wise pattern simply because the values the values jump at a minimum of 5 points as apposed to smaller jumps of 1 or less. 
...
ggplot(data = arbys, aes(sample = total_carb)) + 
  geom_line(stat = "qq")
```
Based on the above Q-Q plot, I would guess that the distribution of carbohydrates at Arbys is relatively normal.
...
ggplot(data= arbys, aes(x=total_carb))+
  geom_bar()
```
With the exception of one outlier, the bar plot confirms that tge distribution appears to be normal as suspected. 
