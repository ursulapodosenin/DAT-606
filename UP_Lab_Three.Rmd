...
A streak length of one means that one hit was made and the number of misses is unkown, while a streak length of 0 means that so hits were made. 
...
The distribution of Kobe's streak length has a positive skew, meaning that the mode is the peak of the distribution while the median and mean are further out to the right. Assuming that typical refers to average,  which could be either the mean median or mode, the most representaive value for Kobe's typical streak length is 0 as that is the mode and the median of the distribution. Kobe's longest streak of baskets was four. 
...
ggplot(data= kobe_streak, aes(x= length))+
  geom_histogram(bins = 5, color= "red", fill= "black")+
    theme_bw()
```
...
```{r set-seed}
set.seed(383940)                 # make sure to change the seed
```
...
# Just as before, the distribution has a positive skew. The typical streak length for this shooter is also 0 and their longest streak is 5 hits. 
```
...
If I were to run the simulation again, I would expect similar results. The distribution will never be normal, regardless of how many times the simulation is run because the probability is not 50/50. There will always be a positive skew as the shooter is more likely to make misses as opposed to hits. 
...
# Kobe Bryant's distribution looks very similar to the simulation distribution suggesting that Kobe Bryant does not have a hot hand. Both distributions have a positive skew with the same mode and median as well as a similar mean.
```
