...
# Yes, there is a trend to the number of girls baptized over the years. Aside from a dip between 1640 and 1660, there appears to be an increasing trend in baptizing girls from 1660 onwards. Every now and then there appear to be a few dips, but the overall number of girls being baptized is increasing.
...
#It seems as though the proportion of boys over time seems to be trending slightly downwards. 
...
# The years included in the data set are 1940-2002. The dimensions of the data frame are 63x3, 63 rows and 3 columns. The column names are "year," "boys," and "girls."
...
# Prior to any manipulation to the data set,  Arbuthnot had 82 observations of 3 variables. The data set, "present," has 63 observations of 3 variables, 19 less observations less than Arbuthnot. 
...
ggplot(present, aes(year)) + 
  geom_line(aes(y = boys, colour = "boys")) + 
...
# The proportion of boys born over time seems to be trening upwards. Yes, Arbuthnot's observation about boys being born in greater proportion than girls is true.
...
# The year 1961 saw the most total number of births in the U.S.

```{r}
present<- present %>% 
mutate (total_births= boys + girls)
max(present$total_births)
which(present$total_births == 4268326, arr.ind=TRUE)
```
...
* * *
 ...
 rmarkdown::render("UP_Lab_One.Rmd", output_format = "pdf_document")