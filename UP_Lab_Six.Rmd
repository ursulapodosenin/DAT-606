...
# Roughly 3% of people texted while driving every day in the past 30 days and never wore helmets.
```
...
rr
margin_of_error <- 2 * sqrt(text_30_and_no_helmet * (1 - text_30_and_no_helmet)/rr)
margin_of_error
```
...
# Physical activity
physical_activity<- yrbss$physically_active_7d
more_active <-yrbss |>
  filter( physically_active_7d > 4) |>
  nrow()

proportion_physical_activity <- more_active/rr
proportion_physical_activity

margin_of_error_two <- 2 * sqrt(proportion_physical_activity * (1 - proportion_physical_activity)/rr)
margin_of_error_two

c(proportion_physical_activity - margin_of_error_two, proportion_physical_activity + margin_of_error_two)
```

Here, we are looking at how many youths exercise more than 4 days a week, which we are counting as success. The confidence interval formula for the proportion of the population who exercises every day is [propotion - margin of error, propotion + margin of error] = 0.4471701 0.4642633. In this scenario , we are 95% confident that the true population proportion of individuals that exercises at least 4 days a week is between 45% and 47%.
```{r}

# Strength Training
strength_training<- yrbss$strength_training_7d
more_strength <-yrbss |>
  filter(strength_training_7d > 4) |>
  nrow()

proportion_strength_training <- more_strength/rr
proportion_strength_training

margin_of_error_three <- 2 * sqrt(proportion_strength_training * (1 - proportion_strength_training)/rr)
margin_of_error_three

c(proportion_strength_training - margin_of_error_three, proportion_strength_training + margin_of_error_three)
```
Here, we are looking at how many youths strength train more than 4 days a week, which we are counting as success. The confidence interval formula for the proportion of the population who exercises every day is [propotion - margin of error, propotion + margin of error] = 0.2816238 to 0.2971880. In this scenario , we are 95% confident that the true population proportion of individuals that strength train at least 4 days a week is between 28% and 30%.

## How does the proportion affect the margin of error?
When using a sample proportion, the margin of error increases and the proportion approaches 50% and decreases as the proportion approaches 0% or 100%. The standard error is greatest when the proportion is near 50%.
...
n <- 1000
p <- seq(from = 0, to = 1, by = 0.01)
margin_of_error_four <- 2 * sqrt(p * (1 - p)/n)
p_and_me <- data.frame(p = p, margin_of_error_four = margin_of_error_four)
ggplot(data = p_and_me, aes(x = p, y = me)) + 
  geom_line() +
  labs(x = "Population Proportion", y = "Margin of Error")
```
As expected, the margin of error is maximized at 50% of the population proportion. 
...
```
The distribution appears to ve normal, with a center around the mean, and a failry even spread on both sides. 
...
```
Increasing the sample size makes the histogram appear even more normally distributed. 
...
```
The distribution appears to be normal with a more even spread on both sides as compared to the above examples.
...
sleep_and_strength
```
H0: There is no relationship between hours of sleep and strength training.
H1: Those who sleep more than 10 hours a week are more likely to strength train every day.
Based on the value of 0.13, which is greater than 0.05, we reject the null hypothesis and conclude that people who sleep 10+ hours per day aren't more likely to strength train everyday of the week.
...
A type I error is a false positive where a true null hypothesis is rejected.
...
pproportion <-0.5
alpha_value <-1.96
measure_of_error_six <-0.01
number<- alpha_value^2*pproportion*(1-pproportion)/measure_of_error_six^2
number
```
The sample would have to contain at least 9604 people.
* * *