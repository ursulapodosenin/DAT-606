...
The wider the bin width, the harder it is to see the variable differences in departure delays. When the bin width is 15 as compared to 30 or 150, it is easier to a more representative distribution of departure delays. 
...
sfo_feb_flights<- (nycflights %>%
  filter(dest == "SFO", month == 2))
sfo_feb_flights<- as.data.frame(sfo_feb_flights)
sfo_feb_flights

# 68 flights are headed to SFO in in February.
```
...
# The distribution of the arrival delays has a right skew.
```
...
# Carriers DL and UA have the most variable delays according to their interquartile range values of 22.00, which is the highest among the carriers. 
...
# The mean gives you the average of all the departure delays while the median gives you the center most value. Given the option to choose between the median and the mean, I think choosing the median is more representative as it is less sensitive to outliers.  
```
...
Simply based on time departure percentage, I would choose to fly out of LGA as it has the highest on-tim departure dates
...
nycflights<- nycflights|>
  mutate(air_time_mph= air_time/60,
         avg_speed= distance/air_time_mph)
```
...
ggplot(data= nycflights, aes(x= avg_speed, y= distance))+
  geom_point(color= 'blue', size= .25, shape= 'triangle')
```
